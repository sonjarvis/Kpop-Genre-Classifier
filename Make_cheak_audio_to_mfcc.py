...
DATASET_PATH = "CheakMusic"
CSV_PATH = "CheakMusic/Cheak_Data_Mfcc.csv"
SAMPLE_RATE = 22050
TRACK_DURATION = 30  # sec
SAMPLES_PER_TRACK = SAMPLE_RATE * TRACK_DURATION  # 한 track의 sample 수


def save_mfcc(dataset_path, csv_path, n_mfcc, n_fft, hop_length, n_segments):
    ...
    data = {"mapping": [], "labels": [], "mfcc": [], "title": []}
    ...
    with open(csv_path, "w") as fp:
        ...
        for tData in mfcc:
            csvwriter.writerow(tData) # 한번에 13개 다 들어가게 2차원 배열로 때려, row말고 col 옆으로 삽입, 이미 잇을때 처리
        csvwriter.writerow(filenames)
        # for line in data:
        #     csvwriter.writerow(data.get(line))

        # json.dump(data, fp, indent=4)   # [발라드, 댄스, 포크, 랩/힙합, 인디음악, 트로트]


if __name__ == "__main__":
    save_mfcc(DATASET_PATH, CSV_PATH, 13, 2048, 512, 5)